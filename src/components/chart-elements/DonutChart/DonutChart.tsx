...
const DonutChart = React.forwardRef<HTMLDivElement, DonutChartProps>((props, ref) => {
    ...
    const categories = Array.from(new Set(data.map((d) => d[index])));
    const defaultActiveIndex = defaultActiveCategory ? categories.indexOf(defaultActiveCategory) : undefined;
    const [activeIndex, setActiveIndex] = React.useState<number | undefined>(defaultActiveIndex);
